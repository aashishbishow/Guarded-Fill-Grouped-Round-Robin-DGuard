...
struct Bench {
    ...
    uint64_t epoch_origin_ns=0, measure_start_ns=0, measure_end_ns=0;
    ...
    void worker(WorkerCtx &c){
        ...
        while(true){
            ...
            if(policy=="dguard"){
                while(true){
                    uint64_t t=now_ns();
                    uint64_t base=epoch_origin_ns; uint64_t epoch=(base && t>base)?((t-base)/(uint64_t)std::max(1,epoch_us*1000)):0;
                    ...
                    uint64_t end=now_ns();
                    if(end>=measure_start_ns && end<measure_end_ns){ c.metrics.waits_ms.push_back((st-arr)/1e6); c.metrics.meals_recorded++; }
                    c.total_meals++;
                    break;
                }
